...
FUZZ_TARGET(coinselection)
{
    ...
    // Note: Dash uses different function signatures than Bitcoin, so we adapt the calls
    const auto result_bnb = SelectCoinsBnB(group_pos, target, coin_params.m_cost_of_change);
    if (result_bnb) {
    ...
    if (result_srd) {
        ...
        result_srd->ComputeAndSetWaste(coin_params.m_cost_of_change);
    ...
    if (result_knapsack) {
        ...
        result_knapsack->ComputeAndSetWaste(coin_params.m_cost_of_change);
