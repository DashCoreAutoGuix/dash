...
bool IsBlockMutated(const CBlock& block, bool check_witness_root)
{
    ...
    if (block.vtx.empty() || !block.vtx[0]->IsCoinBase()) {
        ...
        return std::any_of(block.vtx.begin(), block.vtx.end(),
                           [](auto& tx) { return GetSerializeSize(*tx, CTransaction::CURRENT_VERSION) == 64; });
    } else {
    ...
    // Since Dash doesn't have witness commitments, we skip witness malleation checks
    // when check_witness_root is true, just return false (not mutated)
    return false;
}
