...
// `const` so that `CValidationInterface` clients (which are given a `const ChainstateManager*`)
// can call it.
//
    ...
    try {
    {
        ...
        if (fPruneMode && (m_blockman.m_check_for_pruning || nManualPruneHeight > 0) && !fReindex) {
            ...
            if (nManualPruneHeight > 0) {
                ...
                m_blockman.FindFilesToPrune(setFilesToPrune, m_params.PruneAfterHeight(), m_chain.Height(), last_prune, m_chainman.IsInitialBlockDownload());
...
bool CChainState::InvalidateBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    if (pindex_was_in_chain) {
        uiInterface.NotifyBlockTip(GetSynchronizationState(m_chainman.IsInitialBlockDownload()), to_mark_failed->pprev);
    }
    return true;
}
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    if (!m_chainman.IsInitialBlockDownload() && m_chain.Tip() == pindex->pprev)
