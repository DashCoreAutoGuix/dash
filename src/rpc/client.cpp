...
public:
    ...
    {
        return members.count({method, param_idx}) > 0 ? ParseNonRFCJSONValue(arg_value) : UniValue(std::string(arg_value));
    }

    /** Return arg_value as UniValue, and first parse it if it is a non-string parameter */
    UniValue ArgToUniValue(std::string_view arg_value, const std::string& method, const std::string& param_name)
    {
        return membersByName.count({method, param_name}) > 0 ? ParseNonRFCJSONValue(arg_value) : UniValue(std::string(arg_value));
    }
};
