...
PartiallySignedTransaction ProcessPSBT(const std::string& psbt_string, const CoreContext& context, const HidingSigningProvider& provider, int sighash_type, bool finalize)
{
    ...
    // RemoveUnnecessaryTransactions(psbtx, /*sighash_type=*/1);  // Not available in Dash
...
static RPCHelpMan descriptorprocesspsbt()
{
    return RPCHelpMan{"descriptorprocesspsbt",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    RPCTypeCheck(request.params, {UniValue::VSTR, UniValue::VARR, UniValue::VSTR, UniValue::VBOOL, UniValue::VBOOL}, true);
    ...
    const PartiallySignedTransaction& psbtx = ProcessPSBT(
        request.params[0].get_str(),
        request.context,
        HidingSigningProvider(&provider, /*hide_secret=*/false, /*hide_origin=*/!bip32derivs),
        sighash_type,
        finalize);

    // Check whether there are any complete inputs
    bool complete = true;
