...
static RPCHelpMan importmempool()
{
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            // Call Dash's LoadMempool with the new path parameter
            if (!::LoadMempool(mempool, chainstate, fsbridge::fopen, load_path,
                               use_current_time.isNull() ? true : use_current_time.get_bool(),
                               apply_fee_delta.isNull() ? false : apply_fee_delta.get_bool(),
                               apply_unbroadcast.isNull() ? false : apply_unbroadcast.get_bool())) {
