...
{
    return m_chainman.ActiveChain().Tip()->GetBlockTime() > GetTime<int64_t>() - m_chainparams.GetConsensus().nPowTargetSpacing * 20;
}
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        if (!state.fSyncStarted && CanServeBlocks(*peer) && !fImporting && !fReindex && pto->CanRelay()) {
            ...
            if ((nSyncStarted == 0 && sync_blocks_and_headers_from_peer) || m_chainman.m_best_header->GetBlockTime() > GetTime<int64_t>() - nMaxTipAge) {
                ...
                if (MaybeSendGetHeaders(*pto, msg_type, m_chainman.ActiveChain().GetLocator(pindexStart), *peer)) {
                    ...
                    state.m_headers_sync_timeout = current_time + HEADERS_DOWNLOAD_TIMEOUT_BASE +
                        (
                         ...
                         (GetTime<int64_t>() - m_chainman.m_best_header->GetBlockTime()) / consensusParams.nPowTargetSpacing
                        );
                    nSyncStarted++;
                }
            }
        ...
        if (state.fSyncStarted && state.m_headers_sync_timeout < std::chrono::microseconds::max()) {
            ...
            if (m_chainman.m_best_header->GetBlockTime() <= GetTime<int64_t>() - nMaxTipAge) {
