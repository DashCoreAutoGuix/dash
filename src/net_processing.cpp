...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        if (!state.fSyncStarted && CanServeBlocks(*peer) && !fImporting && !fReindex && pto->CanRelay()) {
            ...
            if ((nSyncStarted == 0 && sync_blocks_and_headers_from_peer) || m_chainman.m_best_header->GetBlockTime() > GetAdjustedTime() - m_chainman.m_options.max_tip_age.count()) {
        ...
        if (state.fSyncStarted && state.m_headers_sync_timeout < std::chrono::microseconds::max()) {
            ...
            if (m_chainman.m_best_header->GetBlockTime() <= GetAdjustedTime() - m_chainman.m_options.max_tip_age.count()) {
