...
void PeerManagerImpl::ProcessCompactBlockTxns(CNode& pfrom, Peer& peer, const BlockTransactions& block_transactions)
{
    ...
    const CNetMsgMaker msgMaker(pfrom.GetCommonVersion());
    {
        ...
        std::map<uint256, std::pair<NodeId, std::list<QueuedBlock>::iterator> >::iterator it = mapBlocksInFlight.find(block_transactions.blockhash);
        ...
        if (status == READ_STATUS_INVALID) {
            RemoveBlockRequest(block_transactions.blockhash); // Reset in-flight state in case Misbehaving does not result in a disconnect
            ...
            // updated, etc.
            RemoveBlockRequest(block_transactions.blockhash); // it is now an empty pointer
            fBlockRead = true;
    ...
    const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (fProcessBLOCKTXN) {
            ...
            txn.blockhash = pindex->GetBlockHash();
