...
public:
    ...
    {
        return AppInitBasicSetup(args()) && AppInitParameterInteraction(args()) && AppInitSanityChecks() &&
...
public:
    ...
    {
        limit_ancestor_count = Assert(m_node.args)->GetIntArg("-limitancestorcount", DEFAULT_ANCESTOR_LIMIT);
        limit_descendant_count = Assert(m_node.args)->GetIntArg("-limitdescendantcount", DEFAULT_DESCENDANT_LIMIT);
    }
    bool checkChainLimits(const CTransactionRef& tx) override
    {
        ...
        auto limit_ancestor_count = Assert(m_node.args)->GetIntArg("-limitancestorcount", DEFAULT_ANCESTOR_LIMIT);
        auto limit_ancestor_size = Assert(m_node.args)->GetIntArg("-limitancestorsize", DEFAULT_ANCESTOR_SIZE_LIMIT) * 1000;
        auto limit_descendant_count = Assert(m_node.args)->GetIntArg("-limitdescendantcount", DEFAULT_DESCENDANT_LIMIT);
        auto limit_descendant_size = Assert(m_node.args)->GetIntArg("-limitdescendantsize", DEFAULT_DESCENDANT_SIZE_LIMIT) * 1000;
    ...
    {
        if (!m_node.mempool) return {};
        return m_node.mempool->GetMinFee(Assert(m_node.args)->GetIntArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) * 1000000);
    }
