                                      ...
                                      uint64_t ancestor_size_limit, uint64_t ancestor_count_limit)
{
    ...
    for (const CTxMemPoolEntry& descendant : descendants) {
        if (!setExclude.count(descendant.GetTx().GetHash())) {
            ...
            if (descendant.GetCountWithAncestors() > ancestor_count_limit || descendant.GetSizeWithAncestors() > int64_t(ancestor_size_limit)) {
                                                  ...
                                                  std::string &errString) const
{
    ...
    while (!staged_ancestors.empty()) {
        ...
        if (stageit->GetSizeWithDescendants() + static_cast<uint64_t>(entry_size) > limitDescendantSize) {
