...
private:
    ...
    bool Fill() {
        ...
        if (nBytes == 0) {
            throw std::ios_base::failure{m_src.feof() ? "CBufferedFile::Fill: end of file" : "CBufferedFile::Fill: fread failed"};
        }
        nSrcPos += nBytes;
        return true;
    }
...
public:
    CBufferedFile(CAutoFile& file, uint64_t nBufSize, uint64_t nRewindIn)
