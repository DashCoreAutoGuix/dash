...
inline CTxMemPoolEntry::CTxMemPoolEntry(const CTransactionRef& tx, CAmount fee,
                                        int64_t time, unsigned int entry_height,
                                        bool spends_coinbase,
                                        int64_t sigops_count, LockPoints lp)
    : tx{tx},
      ...
      nSigOpCountWithAncestors{sigops_count} {}

inline size_t CTxMemPoolEntry::GetTxSize() const
{
    return nTxSize;
}

inline void CTxMemPoolEntry::UpdateDescendantState(int64_t modifySize, CAmount modifyFee, int64_t modifyCount)
{
...
inline void CTxMemPoolEntry::UpdateAncestorState(int64_t modifySize, CAmount modifyFee, int64_t modifyCount, int64_t modifySigOps)
{
    ...
    assert(int64_t(nSigOpCountWithAncestors) >= 0);
}

inline void CTxMemPoolEntry::UpdateModifiedFee(int64_t fee_diff)
{
...
inline void CTxMemPoolEntry::UpdateLockPoints(const LockPoints& lp)
{
    lockPoints = lp;
}
...
struct mempoolentry_wtxid
{
    ...
    result_type operator() (const CTxMemPoolEntry &entry) const
    {
        return entry.GetTx().GetHash();  // Dash doesn't have SegWit witness hash
    }

    result_type operator() (const CTransactionRef& tx) const
    {
        return tx->GetHash();  // Dash doesn't have SegWit witness hash
    }
};
