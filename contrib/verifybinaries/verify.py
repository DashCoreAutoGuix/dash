...
signature file SHA256SUMS.asc from github.com/dashpay/dash and
dashcore-binaries.thepasta.org and compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/dashpay/guix.sigs/tree/main/builder-keys
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the Dash community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    ...
    # remove binaries that are known not to be hosted by github.com/dashpay/dash
...
def main():
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes dashcore-binaries.thepasta.org lags behind github.com/dashpay/dash.)')
    )
