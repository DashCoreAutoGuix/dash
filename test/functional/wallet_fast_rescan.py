...
NUM_DESCRIPTORS = 5  # number of descriptors (4 default ranged pkh ones + 1 fixed non-ranged one, no SegWit in Dash)
...
class WalletFastRescanTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        print(w.importdescriptors([{"desc": descsum_create(f"pkh({fixed_key.privkey})"), "timestamp": "now"}]))
        ...
        for i in range(NUM_BLOCKS):
            ...
            for desc_info in w.listdescriptors()['descriptors']:
                if 'range' in desc_info:
                    ...
                    spk = bytes.fromhex(fixed_key.p2pkh_script)
                    self.log.info(f"-> fixed non-range descriptor address {fixed_key.p2pkh_addr}")
